...
(
    ...
    TITLE_TEMPLATE                TEXT,
    CURRENT_STATUS                PROCESS_STATUS NOT NULL DEFAULT ('ONGOING'),
...
CREATE TYPE USER_TASK_STATUS AS ENUM ('CREATED', 'CLAIMED', 'COMPLETED', 'CANCELLED');
...
(
    ...
    CREATED_ON                        TIMESTAMPTZ DEFAULT now(),
    CURRENT_STATUS                    USER_TASK_STATUS DEFAULT 'CREATED',
    CURRENT_STATUS_TIMESTAMP                 TIMESTAMPTZ DEFAULT now(),
    CURRENT_STATUS_USER_ID                 VARCHAR(100),
    CURRENT_STATUS_ON_BEHALF_OF_USER_ID                 VARCHAR(100),
    CURRENT_STATUS_FIRST_NAME                 VARCHAR(500),
    CURRENT_STATUS_LAST_NAME                 VARCHAR(500),
    CURRENT_STATUS_USER_ORG                 VARCHAR(500),
    JUSTIFICATION                     TEXT
);
...
CREATE TABLE T_PROCESS_RUNNING_STATUS_LOG
(
...
CREATE INDEX IDX_PROCESS_RUNNING_STATUS_LOG
    ON T_PROCESS_RUNNING_STATUS_LOG (PROCESS_INSTANCE_ID);
...
CREATE OR REPLACE FUNCTION FUNC_UPDATE_PROCESS_RUNNING_STATUS()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER TRG_UPDATE_PROCESS_RUNNING_STATUS
    AFTER INSERT ON T_PROCESS_RUNNING_STATUS_LOG
    FOR EACH ROW
    EXECUTE FUNCTION FUNC_UPDATE_PROCESS_RUNNING_STATUS();