CREATE TABLE T_STATIC_TRANSLATION
(
    object_type     TEXT         NOT NULL,
    object_id       VARCHAR(100) NOT NULL,
    attribute_name  TEXT         NOT NULL,
    language_code   VARCHAR(10)  NOT NULL,
    translated_text TEXT         NOT NULL,
    is_default      BOOLEAN     DEFAULT FALSE,
    updated_at      TIMESTAMPTZ DEFAULT now(),

    PRIMARY KEY (object_type, object_id, attribute_name, language_code)
);

CREATE TABLE T_BABEL_TRANSLATION
(
    urn             TEXT        NOT NULL,
    language_code   VARCHAR(10) NOT NULL,
    translated_text TEXT        NOT NULL,
    updated_at      TIMESTAMPTZ DEFAULT now(),
    is_default      BOOLEAN     DEFAULT FALSE,
    PRIMARY KEY (urn, language_code)
);

CREATE TABLE T_BABEL_TRANSLATION_LINK
(
    object_type    VARCHAR(50)  NOT NULL,
    object_id      VARCHAR(100) NOT NULL,
    attribute_name TEXT         NOT NULL,
    urn            TEXT         NOT NULL,
    updated_at     TIMESTAMPTZ DEFAULT now(),

    PRIMARY KEY (object_type, object_id, attribute_name)
);

CREATE INDEX idx_t_static_translation_object
    ON t_static_translation (object_type, object_id);

CREATE INDEX IDX_BABEL_TRANSLATION_LINK
    ON T_BABEL_TRANSLATION_LINK (object_type, object_id);

CREATE TYPE PROCESS_STATUS AS ENUM ('ONGOING', 'COMPLETED', 'PAUSED', 'CANCELLED');

CREATE TABLE T_PROCESS
(
    PROCESS_INSTANCE_ID           VARCHAR(100)   NOT NULL PRIMARY KEY,
    DOMAIN_KEY                    VARCHAR(255),
    PROVIDER_ID                   VARCHAR(255),
    PROCESS_TYPE_KEY              VARCHAR(1000),
    PROCESS_TYPE_ID               VARCHAR(1000),
    CREATED_ON                    TIMESTAMPTZ             DEFAULT now(),
    BUSINESS_STATUS               TEXT,
    BUSINESS_DOMAIN_ID            TEXT,
    RESPONSIBLE_ORGANISATION_ID   VARCHAR(100),
    RESPONSIBLE_ORGANISATION_CODE VARCHAR(100),
    RESPONSIBLE_USER_ID   VARCHAR(100),
    SECURITY_APPLICATION_ID       TEXT,
    SECURITY_SECUNDA_TASK         TEXT,
    SECURITY_SCOPE_TYPE_ID        TEXT,
    SECURITY_SCOPE_ID             TEXT,
    SECURITY_ORGANISATION_ID      TEXT,
    PARENT_PROCESS_INSTANCE_ID                     VARCHAR(50),
    STATUS                    PROCESS_STATUS DEFAULT 'ONGOING',
    NEEDS_REINDEX                 BOOLEAN                 DEFAULT FALSE,
    SEARCH_VECTOR                 TSVECTOR,
    RESULT_CARD                   TEXT
);

CREATE INDEX IDX_PROCESS_SEARCH_VECTOR ON T_PROCESS USING GIN (search_vector);

CREATE TABLE T_PROCESS_CONFIG
(
    PROCESS_INSTANCE_ID VARCHAR(100) NOT NULL PRIMARY KEY,
    TYPE TEXT,
    RESULT_CARD TEXT,
    FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS(PROCESS_INSTANCE_ID) ON DELETE CASCADE
);

CREATE INDEX IDX_PROCESS_PARENT_ID
    ON T_PROCESS (PARENT_PROCESS_INSTANCE_ID);

CREATE INDEX IDX_PROCESS_STATUS
    ON T_PROCESS (STATUS);

CREATE TABLE T_PROCESS_FAVOURITE
(
    PROCESS_INSTANCE_ID           VARCHAR(100),
    USER_ID VARCHAR(100),

    PRIMARY KEY (PROCESS_INSTANCE_ID, USER_ID),
    FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS (PROCESS_INSTANCE_ID) ON DELETE CASCADE
);

CREATE TABLE T_PROCESS_BUSINESS_STATE
(
    PROCESS_INSTANCE_ID VARCHAR(100) NOT NULL,
    BUSINESS_STATE      VARCHAR(255),
    TIMESTAMP           TIMESTAMPTZ  NOT NULL,

    PRIMARY KEY (PROCESS_INSTANCE_ID, BUSINESS_STATE),
    FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS (PROCESS_INSTANCE_ID) ON DELETE CASCADE
);

CREATE INDEX IDX_PROCESS_BUSINESS_STATE
    ON T_PROCESS_BUSINESS_STATE (PROCESS_INSTANCE_ID);

CREATE TYPE USER_TASK_STATUS AS ENUM ('CREATED', 'ASSIGNED', 'COMPLETED', 'CANCELLED');

CREATE TABLE T_USER_TASK
(
    TASK_INSTANCE_ID                  VARCHAR(100) PRIMARY KEY,
    PROCESS_INSTANCE_ID               VARCHAR(100) NOT NULL,
    PROVIDER_ID                       VARCHAR(100),
    DOMAIN_KEY                        VARCHAR(100),
    TASK_TYPE_KEY                     VARCHAR(100),
    TASK_TYPE_ID                     VARCHAR(500),
    CREATED_ON                        TIMESTAMPTZ DEFAULT now(),
    STATUS                    USER_TASK_STATUS DEFAULT 'CREATED',
    ASSIGNED_ON                        TIMESTAMPTZ,
    ASSIGNED_TO                 VARCHAR(100),
    ASSIGNED_TO_ON_BEHALF_OF    VARCHAR(100),
    ASSIGNED_TO_FIRST_NAME      VARCHAR(500),
    ASSIGNED_TO_LAST_NAME      VARCHAR(500),
    ASSIGNED_TO_USER_ORG       VARCHAR(100),
    COMPLETED_ON                 TIMESTAMPTZ,
    COMPLETED_BY                 VARCHAR(100),
    COMPLETED_BY_ON_BEHALF_OF    VARCHAR(100),
    COMPLETED_BY_FIRST_NAME      VARCHAR(500),
    COMPLETED_BY_LAST_NAME      VARCHAR(500),
    COMPLETED_BY_USER_ORG       VARCHAR(100),
    CANCELLED_ON                TIMESTAMPTZ,
    JUSTIFICATION                     TEXT
);

CREATE INDEX IDX_USER_TASK_PROCESS_INSTANCE_ID
    ON T_USER_TASK (PROCESS_INSTANCE_ID);

CREATE INDEX IDX_USER_TASK_STATUS
    ON T_USER_TASK (STATUS);

CREATE TYPE SERVICE_TASK_STATUS AS ENUM ('CREATED', 'COMPLETED', 'FAILED', 'LOCKED', 'CANCELLED');

CREATE TABLE T_SERVICE_TASK
(
    TASK_INSTANCE_ID    VARCHAR(100) PRIMARY KEY,
    PROCESS_INSTANCE_ID VARCHAR(100) NOT NULL,
    TASK_TYPE_KEY       VARCHAR(100),
    PROVIDER_ID                     VARCHAR(100),
    DOMAIN_KEY                     VARCHAR(100),
    PROCESS_TYPE_KEY                     VARCHAR(100),
    STATUS              SERVICE_TASK_STATUS,
    CREATED_ON          TIMESTAMPTZ,
    COMPLETED_ON        TIMESTAMPTZ,
    CANCELLED_ON        TIMESTAMPTZ,
    LOCKED_ON           TIMESTAMPTZ,
    FAILED_ON           TIMESTAMPTZ,
    ERROR_MSG           TEXT,
    INFO_MSG            TEXT
);

CREATE INDEX IDX_SERVICE_TASK
    ON T_SERVICE_TASK (PROCESS_INSTANCE_ID);

CREATE INDEX IDX_SERVICE_TASK_STATUS
    ON T_SERVICE_TASK (STATUS);

CREATE TABLE T_PROCESS_VARIABLE
(
    PROCESS_INSTANCE_ID VARCHAR(100),
    NAME                VARCHAR(500),
    VALUE_TYPE          VARCHAR(20),
    MIME_TYPE           VARCHAR(255),
    VALUE_STRING        TEXT,
    VALUE_INTEGER       INTEGER,
    VALUE_LONG          NUMERIC(19, 0),
    VALUE_DOUBLE        NUMERIC(19, 5),
    VALUE_BOOLEAN       BOOLEAN,
    VALUE_JSON          JSONB,
    VALUE_BYTE          BYTEA,
    VALUE_DATE          TIMESTAMPTZ,
    CREATED_ON          TIMESTAMPTZ DEFAULT now(),
    UPDATED_ON          TIMESTAMPTZ DEFAULT now(),

    PRIMARY KEY (PROCESS_INSTANCE_ID, NAME),
    FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS (PROCESS_INSTANCE_ID) ON DELETE CASCADE
);

CREATE INDEX IDX_PROCESS_VARIABLE
    ON T_PROCESS_VARIABLE (PROCESS_INSTANCE_ID);

CREATE INDEX IDX_PROCESS_VARIABLE_2
    ON T_PROCESS_VARIABLE (PROCESS_INSTANCE_ID, NAME);

CREATE INDEX IDX_PROCESS_VARIABLE_3
    ON t_process_variable (name, value_string)
    WHERE value_string IS NOT NULL;

CREATE INDEX IDX_PROCESS_VARIABLE_4
    ON t_process_variable (name, value_integer)
    WHERE value_integer IS NOT NULL;

CREATE INDEX IDX_PROCESS_VARIABLE_5
    ON t_process_variable (name, value_long)
    WHERE value_long IS NOT NULL;

CREATE INDEX IDX_PROCESS_VARIABLE_6
    ON t_process_variable (name, value_double)
    WHERE value_double IS NOT NULL;

CREATE INDEX IDX_PROCESS_VARIABLE_7
    ON t_process_variable (name, VALUE_BOOLEAN)
    WHERE value_boolean IS NOT NULL;

CREATE INDEX IDX_PROCESS_VARIABLE_8
    ON t_process_variable (name, value_date)
    WHERE value_date IS NOT NULL;

CREATE TABLE T_PROCESS_TAG
(
    process_instance_id VARCHAR(100),
    catalog_id          TEXT,
    tag_id              TEXT,

    PRIMARY KEY (process_instance_id, catalog_id, tag_id),
    FOREIGN KEY (process_instance_id) REFERENCES T_PROCESS (process_instance_id) ON DELETE CASCADE
);

CREATE VIEW V_RESOLVED_TRANSLATION AS
-- Internal translations
SELECT 'static' AS source,
       t.object_type,
       t.object_id,
       t.attribute_name,
       t.language_code,
       t.translated_text,
       t.is_default,
       NULL     AS urn,
       t.updated_at
FROM T_STATIC_TRANSLATION t
UNION ALL
-- External translations
SELECT 'babel' AS source,
       l.object_type,
       l.object_id,
       l.attribute_name,
       e.language_code,
       e.translated_text,
       e.is_default,
       l.urn,
       e.updated_at
FROM T_BABEL_TRANSLATION_LINK l
         JOIN T_BABEL_TRANSLATION e ON l.urn = e.urn;


-- METRO_LINE
CREATE TABLE T_METRO_LINE_FILE
(
    ID            SERIAL PRIMARY KEY,
    TAXONOMY_PATH TEXT UNIQUE NOT NULL,
    CONTENT       TEXT,
    DESCRIPTION   TEXT
);

CREATE TABLE T_METRO_LINE_MAPPING
(
    TAXONOMY_PATH   TEXT NOT NULL,
    STATION_NAME    TEXT,
    CATEGORY        VARCHAR(50),
    PROCESS_TYPE_ID VARCHAR(500),
    TASK_TYPE_ID    VARCHAR(500),
    FOREIGN KEY (TAXONOMY_PATH) REFERENCES T_METRO_LINE_FILE (TAXONOMY_PATH)
);

CREATE INDEX IDX_METRO_LINE_MAPPING
    ON T_METRO_LINE_MAPPING (TAXONOMY_PATH);

-- CREATE TABLE T_PORTFOLIO_ITEM
-- (
--     PORTFOLIO_ITEM_ID TEXT NOT NULL PRIMARY KEY,
--     BUSINESS_ID       TEXT
-- );

CREATE TABLE T_PROCESS_PORTFOLIO_ITEM
(
    PORTFOLIO_ITEM_ID   TEXT NOT NULL,
    PROCESS_INSTANCE_ID VARCHAR(100),

    PRIMARY KEY (PORTFOLIO_ITEM_ID, PROCESS_INSTANCE_ID),
    FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS (PROCESS_INSTANCE_ID) ON DELETE CASCADE
);

CREATE INDEX IDX_PROCESS_PORTFOLIO_ITEM
    ON T_PROCESS_PORTFOLIO_ITEM (PROCESS_INSTANCE_ID);



CREATE TABLE T_PROCESS_ACTION_LOG
(
    ACTION_ID SERIAL PRIMARY KEY,
    PROCESS_INSTANCE_ID VARCHAR(100) NOT NULL,
    TIMESTAMP           TIMESTAMPTZ NOT NULL DEFAULT now(),
    ACTION              VARCHAR(15),
    USER_ID           VARCHAR(50),
    ON_BEHALF_OF_USER_ID VARCHAR(50),
    USER_FIRST_NAME             VARCHAR(500),
    USER_LAST_NAME              VARCHAR(500),
    USER_ORG               VARCHAR(200),

    FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS (PROCESS_INSTANCE_ID) ON DELETE CASCADE
);

CREATE INDEX IDX_PROCESS_ACTION_LOG
    ON T_PROCESS_ACTION_LOG (PROCESS_INSTANCE_ID);

CREATE TABLE T_PROCESS_COMMENT
(
  COMMENT_ID VARCHAR(100) PRIMARY KEY,
  PROCESS_INSTANCE_ID VARCHAR(100),
  TASK_INSTANCE_ID VARCHAR(100),
  COMMENT TEXT NOT NULL ,
  TIMESTAMP TIMESTAMPTZ NOT NULL DEFAULT now(),
  USER_ID VARCHAR(50),
  USER_FIRST_NAME VARCHAR(500),
  USER_LAST_NAME VARCHAR(500),
  USER_ORG VARCHAR(200),

  FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES T_PROCESS(PROCESS_INSTANCE_ID) ON DELETE CASCADE,
  FOREIGN KEY (TASK_INSTANCE_ID) REFERENCES T_USER_TASK(TASK_INSTANCE_ID)
);

CREATE INDEX IDX_PROCESS_COMMENT
    ON T_PROCESS_COMMENT (PROCESS_INSTANCE_ID);

CREATE TABLE T_TAXONOMY
(
    PROCESS_TYPE_ID VARCHAR(500),
    TAXONOMY_PATH TEXT,

    PRIMARY KEY (PROCESS_TYPE_ID, TAXONOMY_PATH)
);

CREATE INDEX IDX_TAXONOMY
  ON T_TAXONOMY (PROCESS_TYPE_ID);

CREATE INDEX IDX_TAXONOMY_2
    ON T_TAXONOMY (TAXONOMY_PATH);

CREATE TABLE T_USER_CACHE (

);

CREATE TABLE T_ORGANISATION (
    ID VARCHAR(100),
    CODE TEXT,

    PRIMARY KEY (ID)
);

CREATE
OR REPLACE FUNCTION FUNC_TRG_REINDEX_T_PROCESS_VARIABLE()
RETURNS TRIGGER AS $$
BEGIN
  -- Only update if VALUE_STRING is NOT NULL (i.e., variable is string)
  IF
NEW.VALUE_STRING IS NOT NULL THEN
UPDATE T_PROCESS
SET NEEDS_REINDEX = TRUE
WHERE PROCESS_INSTANCE_ID = NEW.PROCESS_INSTANCE_ID;
END IF;
RETURN NEW;
END;
$$
LANGUAGE plpgsql;

-- CREATE TRIGGER TRG_REINDEX_T_PROCESS_VARIABLE
--     AFTER INSERT OR
-- UPDATE OR
-- DELETE
-- ON T_PROCESS_VARIABLE
--     FOR EACH ROW
--     EXECUTE FUNCTION FUNC_TRG_REINDEX_T_PROCESS_VARIABLE();

-- update search vector
CREATE
OR REPLACE FUNCTION FUNC_BATCH_UPDATE_SEARCH_VECTOR(limit_rows INT DEFAULT 1000)
RETURNS VOID AS $$
DECLARE
r RECORD;
BEGIN
FOR r IN
SELECT PROCESS_INSTANCE_ID
FROM T_PROCESS
WHERE NEEDS_REINDEX = TRUE
ORDER BY PROCESS_INSTANCE_ID LIMIT limit_rows
    FOR
UPDATE SKIP LOCKED
    LOOP
UPDATE T_PROCESS
SET SEARCH_VECTOR = to_tsvector(
        'simple',
        COALESCE((SELECT string_agg(VALUE_STRING, ' ')
                  FROM T_PROCESS_VARIABLE v
                  WHERE PROCESS_INSTANCE_ID = T_PROCESS.PROCESS_INSTANCE_ID
                  AND VALUE_STRING IS NOT NULL), '') || ' ' ||
        COALESCE((SELECT string_agg(translated_text, ' ')
                  FROM V_RESOLVED_TRANSLATION v
                  WHERE OBJECT_ID = T_PROCESS.PROCESS_INSTANCE_ID
                    AND OBJECT_TYPE = 'PROCESS'), '')
                    ),
    NEEDS_REINDEX = FALSE
WHERE PROCESS_INSTANCE_ID = r.PROCESS_INSTANCE_ID;
END LOOP;
END;
$$
LANGUAGE plpgsql;




--- used for testing only
CREATE OR REPLACE FUNCTION FUNC_TEST_UPDATE_SEARCH_VECTOR()
RETURNS VOID AS $$
BEGIN
WITH variable_data AS (
    SELECT
        PROCESS_INSTANCE_ID,
        string_agg(VALUE_STRING, ' ') AS var_text
    FROM T_PROCESS_VARIABLE
    WHERE VALUE_STRING IS NOT NULL
    GROUP BY PROCESS_INSTANCE_ID
),
     translation_data AS (
         SELECT
             OBJECT_ID AS PROCESS_INSTANCE_ID,
             string_agg(translated_text, ' ') AS trans_text
         FROM V_RESOLVED_TRANSLATION
         WHERE OBJECT_TYPE = 'PROCESS'
         GROUP BY OBJECT_ID
     )
UPDATE T_PROCESS p
SET SEARCH_VECTOR = to_tsvector(
        'simple',
        COALESCE(v.var_text, '') || ' ' || COALESCE(t.trans_text, '')
                    )
    FROM variable_data v
  FULL OUTER JOIN translation_data t ON v.PROCESS_INSTANCE_ID = t.PROCESS_INSTANCE_ID
WHERE p.PROCESS_INSTANCE_ID = COALESCE(v.PROCESS_INSTANCE_ID, t.PROCESS_INSTANCE_ID);
END;
$$ LANGUAGE plpgsql;
-- CREATE
-- OR REPLACE FUNCTION FUNC_TEST_UPDATE_SEARCH_VECTOR()
-- RETURNS VOID AS $$
-- DECLARE
-- r RECORD;
-- BEGIN
-- FOR r IN
-- SELECT PROCESS_INSTANCE_ID FROM T_PROCESS ORDER BY PROCESS_INSTANCE_ID FOR UPDATE SKIP LOCKED
--     LOOP UPDATE T_PROCESS SET SEARCH_VECTOR = to_tsvector(
--         'simple',
--         COALESCE((SELECT string_agg(VALUE_STRING, ' ')
--                   FROM T_PROCESS_VARIABLE v
--                   WHERE PROCESS_INSTANCE_ID = T_PROCESS.PROCESS_INSTANCE_ID
--                     AND VALUE_STRING IS NOT NULL), '') || ' ' ||
--         COALESCE((SELECT string_agg(translated_text, ' ')
--                   FROM V_RESOLVED_TRANSLATION v
--                   WHERE OBJECT_ID = T_PROCESS.PROCESS_INSTANCE_ID
--                     AND OBJECT_TYPE = 'PROCESS'), '')
--                     )
--     WHERE PROCESS_INSTANCE_ID = r.PROCESS_INSTANCE_ID;
-- END LOOP;
-- END;
-- $$
-- LANGUAGE plpgsql;